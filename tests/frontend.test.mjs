...
test('absolute-path PowerShell launch resolves the backend from the project root',()=>{
  ...
  assert.match(shortcutInstaller,/Olos-AI Mini Budget Planner\.lnk/);assert.match(shortcutInstaller,/olos-mini-budget\.ico/);
  assert.match(shortcutInstaller,/wscript\.exe/);assert.match(shortcutInstaller,/launch-hidden\.vbs/);
  assert.match(hiddenLauncher,/127\.0\.0\.1:8876\/api\/health/);assert.match(hiddenLauncher,/olos-ai-mini-budget-planner/);assert.match(hiddenLauncher,/WindowStyle Hidden/);
